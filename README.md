...
This is a test repository for string manipulation utilities, timezone services, a configurable counter implementation, a text-based adventure game world location system, and a high-precision Pi calculator.
...
pi = calculate_pi()
print(f"Pi to 13 decimal places: {pi}")  # Output: 3.1415926535897
```

## Pi Calculator

A high-precision Pi calculator that computes Pi to 13 decimal places using the Machin formula.
...
**Features:**
- Calculates Pi to 13 decimal places: 3.1415926535897
- Uses the proven Machin formula for fast convergence
- High-precision arithmetic with Python's `decimal` module
- Pure mathematical computation (no external dependencies)
...
The calculator uses the Machin-like formula: π/4 = 4·arctan(1/5) - arctan(1/239), which provides excellent convergence properties for high-precision calculation. The implementation uses Taylor series expansion of the arctan function with sufficient terms to ensure accuracy to 13 decimal places.

For detailed information about the algorithm, mathematical background, and performance characteristics, see [Pi Calculation Documentation](docs/pi_calculation.md).
